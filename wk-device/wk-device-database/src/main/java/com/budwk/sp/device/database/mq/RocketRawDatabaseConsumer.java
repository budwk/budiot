...
@RocketMQMessageListener(topic = "${wk.device.database-ext.raw-topic:wk.device.uplink.raw}", consumerGroup = "${spring.application.name}-raw", messageModel = MessageModel.CLUSTERING)
public class RocketRawDatabaseConsumer implements RocketMQListener<String> {
    private final ObjectMapper objectMapper;
    private final DeviceDatabaseDispatchService dispatchService;

    public RocketRawDatabaseConsumer(ObjectMapper objectMapper, DeviceDatabaseDispatchService dispatchService) {
