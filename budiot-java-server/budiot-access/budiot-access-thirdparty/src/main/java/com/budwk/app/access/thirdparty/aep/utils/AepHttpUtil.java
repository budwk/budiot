package com.budwk.app.access.thirdparty.aep.utils;

import com.budwk.app.access.thirdparty.aep.model.ApiResult;
import com.budwk.app.access.thirdparty.aep.model.BaseRequest;
...
public class AepHttpUtil {
    ...
    public static ApiResult doRequest(BaseRequest request) {
        try {
            ...
            if (response.isOK()) {
                ...
                return new ApiResult(jsonObject.getInt("code"), msg, jsonObject.get("result"));
            } else {
                return new ApiResult(response.getStatus(), header.get("x-ag-message", "ERROR"), null);
            }
        } catch (Exception e) {
            log.error(e);
