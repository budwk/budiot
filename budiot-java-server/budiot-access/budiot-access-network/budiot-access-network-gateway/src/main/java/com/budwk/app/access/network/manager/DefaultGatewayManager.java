...
public class DefaultGatewayManager implements DeviceGatewayManager {
    ...
    private DeviceGateway createGateway(DeviceGatewayConfiguration configuration) {
        if (ioc.has(configuration.getTransport() + "GatewayBuilder")) {
            DeviceGatewayBuilder gatewayBuilder = ioc.get(DeviceGatewayBuilder.class, configuration.getTransport() + "GatewayBuilder");
            if (null == gatewayBuilder) {
                log.warn("[gateway] 未找到标识为{}的网关构造器失败", configuration.getTransport());
                return null;
            }
