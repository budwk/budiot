 ...
 * @description v-permission="['sys.list','sys.create']
 */
function notHasPermissionDirective(app: App) {
    app.directive('not-permission', {
        mounted(el, binding) {
            const { value } = binding
            if (value && value instanceof Array && value.length>0) {
                const permissions = useUserInfo().permissions
                ...
                throw new Error(`need permission! Like v-not-permission="['sys.list','sys.create']"`)
            }
        }
    })
}
