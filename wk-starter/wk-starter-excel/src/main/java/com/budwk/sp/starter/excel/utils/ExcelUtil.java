...
public class ExcelUtil<T> {
        ...
        try {
            ...
            if (attr.isExport()) {
                ...
                if (StringUtils.isNotEmpty(dateFormat) && value != null) {
                    ...
                    cell.setCellValue(convertByExp(Strings.sNull(value), readConverterExp, separator));
                ...
                addStatisticsData(column, Strings.sNull(value), attr);
            }
        } catch (Exception e) {
    ...
    public void setCellVo(Object value, Excel attr, Cell cell) {
        if (Excel.ColumnType.STRING == attr.cellType()) {
            String cellValue = Strings.sNull(value);
            ...
            if (value != null) {
                cell.setCellValue(StringUtils.contains(Strings.sNull(value), ".") ? Double.parseDouble(Strings.sNull(value, "0")) : Integer.parseInt(Strings.sNull(value, "0")));
            }
        }
    }
    ...
    public String parseDateToStr(String dateFormat, Object val) {
        ...
        if (val instanceof Date) {
            str = Times.format(dateFormat, (Date) val);
    ...
    public void createSubHead() {
        if (isSubList()) {
            ...
            for (Object[] objects : fields) {
                ...
                headCell1.setCellStyle(styles.get(String.format("header_%s_%s", Strings.sNull(attr.headerColor()), Strings.sNull(attr.headerBackgroundColor()))));
                excelNum++;
            }
    ...
    private Map<String, CellStyle> annotationHeaderStyles(Workbook wb, Map<String, CellStyle> styles) {
        ...
        for (Object[] os : fields) {
            Excel excel = (Excel) os[1];
            String key = String.format("header_%s_%s", Strings.sNull(excel.headerColor()), Strings.sNull(excel.headerBackgroundColor()));
    ...
    public Cell createHeadCell(Excel attr, Row row, int column) {
        ...
        cell.setCellStyle(styles.get(String.format("header_%s_%s", Strings.sNull(attr.headerColor()), Strings.sNull(attr.headerBackgroundColor()))));
        if (isSubList()) {
            // 填充默认样式，防止合并单元格样式失效
            sheet.setDefaultColumnStyle(column, styles.get(String.format("data_%s_%s_%s", Strings.sNull(attr.align()), Strings.sNull(attr.color()), Strings.sNull(attr.backgroundColor()))));
            if (attr.needMerge()) {
